...
MCP23S17::MCP23S17(SPIClass& spi, uint8_t address, uint8_t ss) 
  : _spi(spi)
{
...
void MCP23S17::begin() {
  ...
  _spi.begin();                          // Start up the SPI bus
  _spi.setBitOrder(MSBFIRST);            // Set the SPI_1 bit order
  _spi.setDataMode(SPI_MODE0);           //Set the  SPI_2 data mode 0
  _spi.setClockDivider(SPI_CLOCK_DIV16); // Slow speed (72 / 16 = 4.5 MHz SPI_1 speed)
...
void MCP23S17::byteWrite(uint8_t reg, uint8_t value) {      // Accept the register and byte
  ...
  _spi.transfer(OPCODEW | (_address << 1));             // Send the MCP23S17 opcode, chip address, and write bit
  _spi.transfer(reg);                                   // Send the register we want to write
  _spi.transfer(value);                                 // Send the byte
...
void MCP23S17::pinMode(uint8_t pin, uint8_t mode) {  // Accept the pin # and I/O mode
...
void MCP23S17::pinMode(unsigned int mode) {     // Accept the word…
...
unsigned int MCP23S17::digitalRead(void) {       // This function will read all 16 bits of I/O, and return them as a word in the format 0x(portB)(portA)
  unsigned int value = 0;                   // Initialize a variable to hold the read values to be returned
  ::digitalWrite(_ss, LOW);                 // Take slave-select low
  _spi.transfer(OPCODER | (_address << 1));  // Send the MCP23S17 opcode, chip address, and read bit
  _spi.transfer(MCP_GPIOA);                      // Send the register we want to read
...
uint8_t MCP23S17::byteRead(uint8_t reg) {        // This function will read a single register, and return it
  ...
  ::digitalWrite(_ss, LOW);                 // Take slave-select low
  _spi.transfer(OPCODER | (_address << 1));  // Send the MCP23S17 opcode, chip address, and read bit
  _spi.transfer(reg);                        // Send the register we want to read
  value = _spi.transfer(0x00);               // Send any byte, the function will return the read value
  ::digitalWrite(_ss, HIGH);                // Take slave-select high
