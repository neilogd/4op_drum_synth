...
static void OPL3_ChannelWriteC0(opl3_channel *channel, Bit8u data)
{
    ...
    if (channel->chip->newm)
    {
        channel->ch[0] = ((data >> 4) & 0x01) ? ~0 : 0;
        channel->ch[1] = ((data >> 5) & 0x01) ? ~0 : 0;
        channel->ch[2] = ((data >> 6) & 0x01) ? ~0 : 0;
        channel->ch[3] = ((data >> 7) & 0x01) ? ~0 : 0;
    }
    else
    {
        channel->ch[0] = channel->ch[1] = (Bit16u)~0;
        channel->ch[2] = channel->ch[3] = (Bit16u)0;
    }
}
...
void OPL3_Generate(opl3_chip *chip, Bit16s *buf)
{
	...
	for(cc = 0; cc < 4; cc += 2)
	{
		buf[cc+1] = OPL3_ClipSample(chip->mixbuff[cc+1]);
		...
		for (ii = 0; ii < 18; ii++)
		{
			...
			chip->mixbuff[cc] += (Bit16s)(accm & chip->channel[ii].ch[cc]);
		}
		...
		buf[cc] = OPL3_ClipSample(chip->mixbuff[cc]);
		...
		for (ii = 0; ii < 18; ii++)
		{
			...
			chip->mixbuff[cc+1] += (Bit16s)(accm & chip->channel[ii].ch[cc+1]);
		}
...
void OPL3_GenerateResampled(opl3_chip *chip, Bit16s *buf)
{
	...
	samplediff = (chip->rateratio - chip->samplecnt);
    buf[0] = (Bit16s)((chip->oldsamples[0] * samplediff
                     + chip->samples[0] * chip->samplecnt) / chip->rateratio);
    buf[1] = (Bit16s)((chip->oldsamples[1] * samplediff
                     + chip->samples[1] * chip->samplecnt) / chip->rateratio);
    buf[2] = (Bit16s)((chip->oldsamples[2] * samplediff
                     + chip->samples[2] * chip->samplecnt) / chip->rateratio);
    buf[3] = (Bit16s)((chip->oldsamples[3] * samplediff
...
void OPL3_Reset(opl3_chip *chip, Bit32u samplerate)
{
    ...
    for (channum = 0; channum < 18; channum++)
    {
        ...
        chip->channel[channum].ch[0] = 0xffff;
        chip->channel[channum].ch[1] = 0xffff;
        chip->channel[channum].ch[2] = 0x0;
        chip->channel[channum].ch[3] = 0x0;
